...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def interpolate_pos_encoding(self, x, pos_embed, size):
        ...
        """
        npatch = x.shape[1] - 2  # 2 for [CLS] and distillation tokens
        N = pos_embed.shape[1] - 2  # 2 for [CLS] and distillation tokens
        if npatch == N:
            return pos_embed

        class_emb, distil_emb, pos_embed = pos_embed[:, 0], pos_embed[:, 1], pos_embed[:, 2:]
        ...
        pos_embed = pos_embed.permute(0, 2, 3, 1).view(1, -1, dim)
        pos_embed = torch.cat((class_emb.unsqueeze(0), distil_emb.unsqueeze(0), pos_embed), dim=1)
        return pos_embed

    def forward_features(self, x, return_patch_tokens: bool = False):
        ...
        patch_embed_h, patch_embed_w = x.shape[-2] // patch_size, x.shape[-1] // patch_size

        # x: b x 3 x H x W -> b x (h * w) x n_dims
        x = self.patch_embed(x)

        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        dist_token = self.dist_token.expand(B, -1, -1)

        # x: b x (h * w) x n_dims -> b x (2 + h * w) x n_dims
        x = torch.cat((cls_tokens, dist_token, x), dim=1)

        # noel - interpolate positional embeddings
        pos_embed = self.interpolate_pos_encoding(x, self.pos_embed, size=(patch_embed_h, patch_embed_w))
        x = x + pos_embed
        # x = x + self.pos_embed
        x = self.pos_drop(x)
    ...
    def forward(self, x, return_patch_tokens: bool = True):
        ...
        if return_patch_tokens:
            # noel - added
            return x[:, 2:].permute(0, 2, 1).view(B, -1, patch_embed_h, patch_embed_w)
            # return x[:, 2:]
...
if __name__ == '__main__':
    model = dino_small_dist(patch_size=16, pretrained=True)
