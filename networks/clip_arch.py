# copied from https://github.com/openai/CLIP/blob/main/clip/model.py
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, return_attn: bool = True):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        # return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
        attn_output, attn_weights = self.attn(
            x, x, x, need_weights=return_attn, average_attn_weights=False, attn_mask=self.attn_mask
        )
        return attn_output, attn_weights

    def forward(self, x: torch.Tensor, return_attn: bool = True):
        attn_output, attn_weights = self.attention(self.ln_1(x), return_attn=return_attn)
        x = x + attn_output
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_interm_tokens: bool = False, return_attn: bool = True):
        ...
        for resblock in self.resblocks:
            x, attn = resblock(x, return_attn=return_attn)
            list_x.append(x)
        if return_interm_tokens:
            return list_x
        else:
            if return_attn:
                return x, attn
...
class VisionTransformer(nn.Module):
    ...
    # copied from https://github.com/facebookresearch/dino
    def prepare_tokens(self, x):
        ...
        x = self.conv1(x)  # patch linear embedding, [*, width, grid, grid]
    ...
    def interpolate_pos_encoding(self, x, pos_embed, size):
        ...
        pos_embed = pos_embed.permute(0, 2, 3, 1).view(-1, dim)
        pos_embed = torch.cat((class_emb.unsqueeze(dim=0), pos_embed), dim=0)
        return pos_embed  # [None]  # 1 x (1 + N patches) x n_dims

    def forward(
            self,
            x: torch.Tensor,
            return_patch_tokens: bool = True,
            return_interm_tokens: bool = False,
            return_attn: bool = False,
    ):
        # noel: I added return_patch_tokens argument and interpolation of pos emb part
        ...
        pos_emb = self.interpolate_pos_encoding(x, self.positional_embedding.to(x.dtype), size=(h, w))
        x = x + pos_emb
        # x = x + self.positional_embedding
        x = self.ln_pre(x)

        x = x.permute(1, 0, 2)  # NLD -> LND
        x, attn = self.transformer(x, return_interm_tokens=False, return_attn=return_attn)
        ...
        if return_patch_tokens:
            ...
            return patch_tokens, attn
        else:
            if self.proj is not None:
                # self.proj: vision_n_dim x text_n_dim
                cls_token = cls_token @ self.proj
            return cls_token.view(b, -1), attn
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_interm_tokens: bool = False, return_patch_tokens: bool = True):
        return self.visual(
            image.type(self.dtype),
            return_interm_tokens=return_interm_tokens,
    ...
    def forward(self, image, text=None, return_interm_tokens: bool = False):
        x, attn = self.encode_image(image, return_interm_tokens=return_interm_tokens)
        return x
